...
function setBgColor(color) {
  // changes bgColor to specified color
  document.body.style.backgroundColor = color;
}

function setText(text) {
  // sets hint text to specified text
  hint.innerHTML = text;
}
...
function upDown(num, ans) {
  // checks if guess is close to the answer and gives up or down hints
  const diff = num - ans;
  console.log(diff);
  if (diff === 0) {
    setBgColor("#8dcc9e");
...
function checkNum(val, answer) {
  // checks if val is a number or if it's within bounds
